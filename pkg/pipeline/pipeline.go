...
import (
	...
	"github.com/carbonteq/egress-ehancement/pkg/pipeline/input"
	...
	"github.com/carbonteq/egress-ehancement/pkg/pipeline/output"
	"github.com/carbonteq/egress-ehancement/pkg/pipeline/params"
	"github.com/carbonteq/egress-ehancement/pkg/pipeline/sink"
...
func (p *Pipeline) Run(ctx context.Context) *livekit.EgressInfo {
	...
	case params.EgressTypeFileAndStream:
		var err error
		p.FileInfoFS.Location, p.FileInfoFS.FileSize, err = p.storeFile(ctx, p.LocalFilepathFS, p.StorageFilepathFS, p.OutputType)
		if err != nil {
			p.Info.Error = err.Error()
		}
